...
from torch.distributed.fsdp import (
    ...
    ShardingStrategy, MixedPrecision, StateDictType,
)
...
import numpy as np
import random
from .distributed.collective import scatter_to_sequence_parallel_region, gather_from_sequence_parallel_region
...
SUPPORT_DATASETS = [torch.utils.data.Dataset]
try:
    from datasets import arrow_dataset # huggingface datasets
    SUPPORT_DATASETS.append(arrow_dataset.Dataset)
    SUPPORT_DATASETS = tuple(SUPPORT_DATASETS)
except ImportError:
    SUPPORT_DATASETS = tuple(SUPPORT_DATASETS)
...
class LiteGen:
    ...
    def _setup_sequence_parallel(self, model):
        ...
        from models.vid_sd3.sparse_attention import Attention
    ...
    def _load_optimizer(self, model, optimizer, output_folder, filename):
        state_dict = torch.load(os.path.join(output_folder, filename), map_location="cpu")
    ...
    def load_optimizer_state(self, optimizer):
        ...
        if load_from is not None:
            ...
            optimizer = self._load_optimizer(self.model, optimizer, "", load_from)
        else:
    ...
    @torch.no_grad()
    def update_ema(self, model=None, decay=0.9999, only_trainable=True):
        ...
        if self.zero_degree == 3 or not self.ema_cfg.sharded:
            ...
            self.model_ema.update(model, decay, only_trainable)
    ...
    def _initialize_optimizer(self, optimizer):
        ...
        if self.zero_degree == 1:
            ...
            def _optimizer_to_zero(optimizer):
                ...
                if optimizer_cls in SUPPORT_OPTIMIZERS:
                    ...
                    raise NotImplementedError(f"Optimizer {optimizer_cls} is not supported currently. Supported optimizers are {SUPPORT_OPTIMIZERS}")
    ...
    def _initialize_dataset(self, obj):
        assert isinstance(obj, SUPPORT_DATASETS), f"only support dataset with type {SUPPORT_DATASETS}, but got {type(obj)}"
    ...
    def initialize(self, *args):
        ...
        for obj in args:
            ...
            elif isinstance(obj, SUPPORT_DATASETS):
